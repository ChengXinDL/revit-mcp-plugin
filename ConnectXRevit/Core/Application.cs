...
namespace ConnectXRevit.Core
{
    ...
    public class RevitMcpApplication : IExternalApplication
    {
        ...
        // 插件名称常量
        private const string PluginName = "ConnectX Revit";
        ...
        private void AddServerToggleButton(RibbonPanel panel)
        {
            try
            {
                var buttonData = new PushButtonData(
                    ...
                    "ConnectXRevit.Core.MCPServiceConnection"
                )
                {
        ...
        /// 添加AI对话框按钮
        /// </summary>
        private void AddAIChatButton(RibbonPanel panel)
        {
            try
            {
                var buttonData = new PushButtonData(
                ...
                if (pushButton != null)
                {
                    pushButton.ToolTip = "AI Agent服务连接并进行交互";
                }

                _logger.Info("AI交付对话框按钮添加成功");
            }
            catch (Exception ex)
            {
                _logger.Error("AI交付对话框按钮失败: {0}", ex.Message);
            }
        }
        ...
        private BitmapImage LoadImage(string imageName)
        {
            try
            {
                ...
                var uri = new Uri(
                    $"/ConnectXRevit;component/Core/Resources/{imageName}",  // 修正文件夹名拼写（Ressources→Resources）
