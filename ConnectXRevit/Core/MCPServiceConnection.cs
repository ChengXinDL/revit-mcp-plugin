    ...
    public class MCPServiceConnection : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                SocketServerRPC socketService = SocketServerRPC.Instance;
                ...
                {
                    socketService.Initialize(_revitApp);
                    socketService.Start();
                    //StartHttpServer();
                    _logger.Info($"Socket服务已启动\nHTTP服务已启动，监听端口：{HttpServerPort}）");
        ...
        private void InitializeCommandManager()
        {
            ...
            try
            {
                ...
                _logger.Info("CommandManager初始化完成，已加载命令数量: {0}", CommandManagerExtensions.GetLoadedCommandCount(_commandManager));
            }
            catch (Exception ex)
            {
